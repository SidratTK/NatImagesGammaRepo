...
% the values should cross input thresholds at a given size & remain crossed
% for the next 2 sizes. 
...
if numStimuli>1 % to account for case when fixed radius is sent as input
rList = [];
for j = 1:3
    ...
    temp = foo > selectOptions.meanThr(j); % logical 
    % if it returns to sub threshold levels within 2 steps, skip that crossing. STK
    for tt=1:length(temp)-2 
        if temp(tt) && (~temp(tt+1) || (temp(tt+1) && ~temp(tt+2)))
           temp(tt) = false; 
           break;
        end 
    end 
    if ~isempty(find(temp, 1 ))
        rList = cat(2,rList, find(temp, 1 ));
    else
        rList = cat(2,rList, numStimuli+1);
    end
    temp = stdList(j, :) > selectOptions.stdThr(j); % logical 
    for tt=1:length(temp)-2  % STK
        if temp(tt) && (~temp(tt+1) || (temp(tt+1) && ~temp(tt+2))) 
           temp(tt) = false; 
           break;
        end 
...
rIdx = min(rList)-1; % The index of the maximum approximation radius which doesn't deviate beyond threshold
                         % -1 because we checked for radius at which deviation occured. 
else, rIdx=1;
end
if rIdx==0 
    gaborStim.radiusDeg = 0; % doesnt qualify for a patch
else
...
stimParams.radiusDeg = gaborStim.radiusDeg;% radiusMatrixDeg(rIdx);
